...
document
  .getElementById('run-async-problem')
  .addEventListener('click', async () => {
    ...
    await logToConsole('console-async-problem', 'Ordering a pizza...', {
      delay,
    });
    ...
    setTimeout(async () => {
      ...
      await logToConsole(
        'console-async-problem',
        '(2 seconds later...) Pizza is ready! But the value was never captured.',
        { isPromise: true }
      );
      setTimeout(clearAllHighlights, delay);
    }, 2000);
    ...
    await logToConsole('console-async-problem', 'Doing other things...', {
      delay,
    });

    await highlightLine('code-async-problem', 12);
    await logToConsole('console-async-problem', 'Eating pizza: undefined', {
      delay,
      isError: true,
    });
...
document.getElementById('run-promise').addEventListener('click', async () => {
  ...
  setTimeout(async () => {
    ...
    await logToConsole('console-promise', 'Promise created. Status: pending', {
      isPromise: true,
    });
  ...
  setTimeout(async () => {
    ...
    await logToConsole('console-promise', 'Promise resolved!', {
      isPromise: true,
    });
    await highlightLine('code-promise', 12, delay);
    await highlightLine('code-promise', 13, delay);
    await logToConsole('console-promise', 'Eating pizza: 🍕 Pizza is ready!', {
      isPromise: true,
      delay,
    });
    setTimeout(clearAllHighlights, delay);
...
document
  .getElementById('run-async-await')
  .addEventListener('click', async () => {
    toggleButtons(true);
    ...
    setTimeout(async () => {
      ...
      await logToConsole('console-async-await', 'Inside async function.', {
        delay,
      });
      ...
      await logToConsole(
        'console-async-await',
        'Awaiting promise... Function is paused here.',
        { delay, isPromise: true }
      );
      ...
      setTimeout(async () => {
        await logToConsole(
          'console-async-await',
          'Promise resolved! Resuming function.',
          { isPromise: true, delay }
        );
        await highlightLine('code-async-await', 6, delay);
        await logToConsole(
          'console-async-await',
          'Eating pizza: 🍕 Pizza is ready!',
          { isPromise: true, delay }
        );
        await highlightLine('code-async-await', 7, delay);
        await logToConsole('console-async-await', 'Finished dinner.', {
          delay,
        });
    ...
    await logToConsole('console-async-await', 'Doing other things...', {
      delay,
    });
