...
// ==========================================
//
// INSTRUCTIONS: Look for TODO sections throughout this file to complete the
...
function loadBlockbusterScript(forceFail) {
  ...
  // Your task: Create a new Promise that simulates downloading a script
  // from Hollywood servers
  // create a new promise and assign it to a variable using new Promise syntax
  const promise = new Promise((resolve, reject) => {
    // inside of the promise callback function set
    // setTimeout to simulate loading the data with a 3 second duration
    // within the timeout callback
    setTimeout(() => {
      // if forceFail is true
      if (forceFail) {
        //reject with message
        reject('Mission not accomplished');
      } else {
...
function handleScriptLoad(shouldFail = false) {
  ...
  // Your task: Show what happens when you try to use a Promise directly
  // (without .then()) This demonstrates the WRONG way - storing a Promise
  // directly in a variable. Call the Promise function and store the result
  // directly (this won't give us the data)
  const promiseResult = loadBlockbusterScript(shouldFail);
  // Log what we actually get - it will be a Promise object, not the script data
  console.log(
    'Directly assigned Promise without .then() logs promise itself:',
    promiseResult
  );
  // TODO 2 END - Notice this logs a Promise object, not the actual script data

  // TODO 3 START - Demonstrate correct Promise usage
  // Your task: Use .then() and .catch() to properly handle the Promise
  // This demonstrates the RIGHT way - using .then() to handle resolved Promises
  ...
  loadBlockbusterScript(shouldFail)
    ...
    .then((movieScript) => {
      ...
      statusDiv.textContent = 'Script loaded successfully! Ready for action!';
    ...
    // Chain a .catch() to handle any errors
    .catch((error) => {
  ...
  // TODO 3 END - Notice how .then() gets the actual data, and .catch() handles
  // any errors
